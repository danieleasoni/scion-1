...
func (rp *RtrPkt) Validate() *common.Error {
	...
	// FIXME: disabled path requirement for HORNET packets
	if len(rp.HBHExt) > 0 {
		if rp.HBHExt[0].Type() == common.ExtnHORNETType {
			rp.Logger.Info("HORNET: Skip path validation for HORNET packet")
			if err := rp.validatePath(rp.DirFrom); err != nil {
				rp.Logger.Info("HORNET: would have gotten error")
			} else {
				rp.Logger.Info("HORNET: would have been successful!")
			}
		} else {
			if err := rp.validatePath(rp.DirFrom); err != nil {
				return err
