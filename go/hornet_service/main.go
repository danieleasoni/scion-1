...
var (
	id       = flag.String("id", "", "Element ID (Required. E.g. 'hs4-21-9')")
...
func main() {
	...
	setupSignals()
	hsvc, err := NewHornetService(*id, *confDir)
	if err != nil {
	...
	if assert.On {
		log.Info("HORNET service was built with assertions ON.")
	} else {
		log.Info("HORNET service was built with assertions OFF.")
	}
	log.Info("Starting up", "id", *id, "pid", os.Getpid())
	if err := hsvc.Run(); err != nil {
